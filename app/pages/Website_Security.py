...
from src.database
 import save_scan, create_database
    ...
    if url.strip() == "":
        ...
        if "error" in result:
            ...
            with col_title:
                ...
                st.header(
                    result.get("website", "Unknown Website")
                )
            ...
            with c2:
                ...
                st.metric(
                    "🔒 HTTPS",
                    "Enabled ✅" if result.get("https") else "Disabled ❌"
                )

                st.metric(
                    "🔐 SSL Status",
                    result.get("ssl_status", "Unavailable")
                )
            ...
            with c3:
                ...
                st.metric(
                    "📅 SSL Expiry",
                    result.get("ssl_expiry", "Unavailable")
                )
            ...
            if result.get("ssl_status") != "Valid ✅":
                ...
                recommendations.append(
                    "Install or renew a valid SSL certificate."
                )
